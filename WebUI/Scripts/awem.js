awem = function ($) {
    ...
    function parseDate(format, value) {
        ...
        var res = {};
        while (format) {
            var head = getHead(format);
            if (head) {
                format = format.substr(head.length);
                if (format) {
                    // get the str to convert
                    var nextHead = getHead(format);
                    var sepix;
                    if (!nextHead) {
                        var sep = format[0];
                    ...
                    var str = value.substr(0, sepix);
                    value = value.substr(sepix + 1);

                    parsf[head](str, res);
                }
                else {
                    parsf[head](value, res);
                }
            } else {
    ...
    function layDropdownPopup2(o, pop, isFixed, capHeight, opener, setHeight, keepPos, canShrink, chkfulls, minh, popuph, maxph, popup, xy, opt) {
    ...
    function multiselect(o) {
        ...
        if (reor) {
            ...
            function wrap(clone, dragObj) {
                initm = 1;
                placeholder = dragObj.clone().addClass('awe-hl placeh').hide();
        ...
        if (searchOutside) opt.d.srctxt = $searchtxt;
    ...
    function combobox(o) {
        ...
        var $openbtn = $(rbtn('o-cbxbtn o-ddbtn awe-btn o-btn', scaret, 'tabindex="-1"'));
        var docClickRegistered = 0;
        var glrs;
        var closeOnEmpty = readTag(o, "ClsEmpt");
        var api, dd, dropmenu;
        var vprop;
        var contval = se;
    ...
    function autocomplete(o) {
        ...
        var cache = o.c;
        var dd = dropmenu({
            i: o.i, opener: input, srctxt: input, select: onSelect,
            sf: searchFunc, sfo: o, nacc: !cache, minl: o.minl, dl: o.dl, itemFunc: o.itemFunc,
            psf: postSearchFunc, clsempt: 1, ck: o.ck, gval: gval, nom: 1, combo: 1
        });
        ...
        function postSearchFunc(k) {
            if (!contains(k, nonComboSearchKeys)) {
                if (!dd.menu.find('.o-itm:visible').length) {
                    dd.close();
                }
            }
        }
        ...
        function searchFunc(_, info) {
            ...
            var f = o.df ? o.df(term, prm) : $.post(o.u, prm);
            return $.when(f).done(function (data) {
                ...
                return data;
            }).fail(function () { c.t[term] = 0; });
        }

        input.keyup(function (e) {
            var val = input.val();
            var key = which(e);
            if (!val && !contains(key, updownKeys)) {
                dd.close();
            } else if (!dd.isOpen()) {
                if (contains(key, updownKeys)) {
                    dd.open();
    ...
    function dataSource(glrs, cacheKey) {
        return {
            ...
            glr: function () {
                ...
                if (cacheObj) {
                    ...
                    for (var i = 0; i < lrs.length; i++) {
                        if (cacheObj.Keys[kp(lrs[i])] == null) {
                            res.push(lrs[i]);
                        }
                    }
            ...
            addc: function (result, gval) {
                ...
                if (gval && items.length > cacheLimit) {
                    var vals = gval();
                    var nitems = [];
                    var nkeys = {};
    ...
    function dropmenu(o) {
        ...
        var slistctrl = slist($itemscont, { sel: '.o-itm', afs: '.o-ditm', botf: botf });
        ...
        var onItemClick = o.click || function () {
            var $clickedItem = $(this);
            var index = $clickedItem.data('index');

            o.select && o.select(glrs()[index], $clickedItem.data('val'));
        ...
        $dropmenu.on(sclick, '.o-itm', onItemClick)
        ...
        function srctxtKeyup(e) {
            if (!contains(which(e), nonComboSearchKeys) && !e.ctrlKey) {
                ...
                if (closeOnEmpty && !localSearchResCount) {
                    close();
                }
        ...
        function renderMenu() {
            ...
            if ((rs.length > srcThresh || searchFunc) && !noAutoSearch) {
                $menuSearchCont.show();
            } else {
                $menuSearchCont.hide();
            }
            ...
            for (var i = 0; i < rs.length; i++) {
                var item = rs[i];
                res += '<li class="o-itm o-ditm" ' + o.renItAttr(i, item) + '>' + o.renderItemDisplay(item) + '</li>';
            }
        ...
        function toggleOpen(opt) {
            ...
            if (isOpen()) {
                ...
                if (opener) {
                    !(opt.x != null) && $dropmenu.css(sminw, opener.outerWidth() + 'px');
                }
            ...
            if ($dropmenu.hasClass('open')) {
                ...
                function setHeight(poph, maxh, valign) {
                    ...
                    if (valign == 'top') {
                        ...
                        var h = maxh - rest;
                        if (valign != 'fls') {
                            h = Math.min(h, maxDropdownHeight);
                        }

                        oitemsc.css('max-height', h);
                    }
                }

                layDropdownPopup2(o, $dropmenu, isFixed, null, olc, setHeight, 0, 0, chkfulls, isCombo ? comboMinh : menuMinh, 0, maxDropdownHeight, 0, popt && popt.xy);
        ...
        function filter(query, clickedItem) {
            ...
            if (isCombo && showCmbItm) {
                ...
                cmbi.html(o.renderItemDisplay({ c: ec, k: ec })).data('val', comboPref + query);
            }
        ...
        function markMenuSelectedItems() {
            if (o.gval && !o.nom) {
                var val = o.gval();
                var items = glrs();
                $dropmenu.find('.o-ditm')
                    .each(function (i, el) {
                        var checked = containsVal(items[i][vprop], val);
                        if (checked) {
                            $(el).addClass(sselected);
                        } else {
                            $(el).removeClass(sselected);
                        }
                    });
            }
        }
    ...
    function odropdown(o, opt) {
        ...
        var btn = $(rbtn('o-ddbtn o-btn awe-btn"', scaret, 'id="' + o.i + '-awed"'));
    ...
    function dropdownPopup(o) {
        ...
        var api = function () { };
        var itmoved, header, $opener, openerId, fls, popt;
        var btns = p.btns;
    ...
    function gridPageSize(o) {
        ...
        $grid.find('.awe-footer').append('<div class="awe-ajaxradiolist-field o-gpgs" ><input id="' + psi + '" class="awe-val" type="hidden" value="' + o.ps + '" /><div class="awe-display"></div></div>');
        ...
        return function (o) {
            ...
            function hoverFunc($c) {
                return function (dragObj, x, y) {
                    ...
                    if (currhovering != $c) {
                        currhovering = $c;
                        placeh = getRow($c).addClass('awe-hl');
                        ogrow = placeh.clone();
                    }
    ...
    function rendOchk(radio, checked, type, name, value) {
        var checkstr = checked ? 'checked="checked"' : se;
        var nmv = name ? 'value="' + value + '" name="' + name + '"' : se;
        return '<div tabindex="0" class="o-chk ' + (checked ? schked : se) + '" >' +
            (radio ? se : '<div class="o-chkc"><div class="o-chkico"></div></div>') +
            '</div><input type="' + type + '" ' + nmv + ' ' + checkstr + ' style="display:none"/>';
    }
        ...
        function render() {
            ...
            for (var i = 0; i < o.lrs.length; i++) {
                ...
                items += '<li class="o-chkm ' + schkc + '"><label class="awe-label">' +
                     rendOchk(radio, checked, type, o.nm, kp(item)) +
